...
struct utf_traits<CharType, 1> {
  ...
  static code_point decode(Iterator &p, Iterator e) {
    ...
    unsigned char tmp;
    switch (trail_size) {
      case 3:if (NOWIDE_UNLIKELY(p == e)) return incomplete;
        tmp = *p++;
        if (!is_trail(tmp)) return illegal;
        c = (c << 6) | (tmp & 0x3F);
      case 2:if (NOWIDE_UNLIKELY(p == e)) return incomplete;
        tmp = *p++;
        if (!is_trail(tmp)) return illegal;
        c = (c << 6) | (tmp & 0x3F);
      case 1:if (NOWIDE_UNLIKELY(p == e)) return incomplete;
        tmp = *p++;
        if (!is_trail(tmp)) return illegal;
        c = (c << 6) | (tmp & 0x3F);
    }
  ...
  static code_point decode_valid(Iterator &p) {
    ...
    switch (trail_size) {
      case 3:c = (c << 6) | (static_cast<unsigned char>(*p++) & 0x3F);
      case 2:c = (c << 6) | (static_cast<unsigned char>(*p++) & 0x3F);
